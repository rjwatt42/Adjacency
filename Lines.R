...
makeLines<-function(nlines=9,lineLength=0.2,lineWidth=1,lineCol='#000',
  ...
  if (guideLines) {
    for (i in 1:nlines) {
      g<-addG(g,dataLine(data.frame(x=linesX[i],y=linesY+lineLength+guideOffset),linewidth=lineWidth/0.015, colour=lineCol))
      g<-addG(g,dataLine(data.frame(x=linesX[i],y=linesY-lineLength-guideOffset),linewidth=lineWidth/0.015, colour=lineCol))
    }
  }
